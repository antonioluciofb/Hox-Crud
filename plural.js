...
module.exports = (db, name, opts) => {
  ...
  function destroy(req, res, next) {
    const idList = req.params.id.split(',').filter(item => item !== '' || item !== undefined || item !== null);

    function destroyById(id) {
      const resource = db.get(name).removeById(id).value(); // Remove dependents documents
    ...
    let resourceList = idList.map(destroyById); // if has one more id matched -> will not return 404
